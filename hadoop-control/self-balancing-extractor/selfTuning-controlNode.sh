...
str=$(tail -n 1000 ~/hadoop/logs/$logfile | grep "$newtime" | grep 'pending')
newtime="10"
...
#Time
	...
	echo $newtime | awk '{print "Time(s): "$1}' | tee -a controlNodeLog

#Number of idle jobs
	echo $lastline | cut -d "#" -f 2,3,4,5 | sed -e "s=#==g" | cut -d " " -f 1,2,3,4,5,6,7,8 | tee -a controlNodeLog

	echo ""  | tee -a controlNodeLog
	echo "#########################################"  | tee -a controlNodeLog
	echo ""  | tee -a controlNodeLog
fi
